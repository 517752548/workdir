...
namespace Assets.Scripts.DataManagers
{
	public class PlayerArmyManager : Tools.XSingleton<PlayerArmyManager>, IPresist
	{
		...
		{
			//已经召唤了
			if (this._employSoldier.Contains (hero.ID))
				return;
		...
		{
			if (this.Soldiers.ContainsKey (hero.recruit_id))
				return false;
			...
			case EmployCondtionType.GetItem:
				...
				foreach (var i in keyValues) {
					if (!(PlayerItemManager.Singleton.GetItemCount (i) > 0))
						return false;
				}
				return true;
		...
		{
			if (!Soldiers.ContainsKey (Soldier.SoldierID))
				return false;
